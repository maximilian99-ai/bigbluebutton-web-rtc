  ...
  module V1
    class MeetingsController < ApiController
      ...
      def status
        ...
        puts "provider: #{current_provider}"
        puts "current_user: #{current_user}"

        # SQL 쿼리 실행
        # room_id = 'e8800b22-1d15-4fd1-ae1b-cc4d47cc92ce'  # 조회하고 싶은 room의 id
        query = "SELECT * FROM rooms WHERE id='#{@room.id}'"
        #여기서 들어가는 해당룸 password 확인가능
        room_record = ActiveRecord::Base.connection.execute(query)
        
        puts "Queried Room Record: #{room_record.to_a}"
        ...
        settings = RoomSettingsGetter.new(
          ...
          current_user:,
          show_codes: true,
          settings: %w[glRequireAuthentication glViewerAccessCode glModeratorAccessCode glAnyoneCanStart glAnyoneJoinAsModerator glRoomPassword]
        ).call

        puts "Room settings: #{settings.inspect}"
