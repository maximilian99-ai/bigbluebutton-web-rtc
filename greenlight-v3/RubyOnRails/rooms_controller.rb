module Api
  module V1
    class RoomsController < ApiController
      ...
      def index
        # 모든 방을 가져오되, 사용자 정보를 포함하여 쿼리의 효율을 높임
        rooms = Room.includes(:user)
                    .order(online: :desc)
                    .order('last_session DESC NULLS LAST')
                    .search(params[:search])
    
        # 공유된 방 여부 설정 로직은 제거하거나 수정할 수 있습니다
        rooms.map do |room|
          room.shared = SharedAccess.exists?(user_id: current_user.id, room_id: room.id)
        end
      ...
      def create
        ...
        room = Room.new(name: room_params[:name], password: room_params[:password], user_id: room_params[:user_id])
      ...
      def room_params
        params.require(:room).permit(:name, :password, :user_id, :presentation)
      end
    end
  end
end
