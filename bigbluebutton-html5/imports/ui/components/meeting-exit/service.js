...
const intlMessages = defineMessages({
  ...
  notification: {
    id: 'app.meeting-exit.notification',
...
const setStats = (level = -1, type = 'recovery', value = {}) => {
  ...
  addMeetingExit(level, type, value);
};
...
const addMeetingExit = (level, type, value) => {
  const status = level !== -1 ? STATS.level[level] : 'normal';

  makeCall('addMeetingExit', status, type, value);
};
...
const getMeetingExit = () => {
  ...
  const connectionStatus = MeetingExit.find(selector).fetch().map((userStatus) => {
    const {
      userId,
      status,
      statusUpdatedAt,
